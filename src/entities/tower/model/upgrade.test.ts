...
describe('tower upgrade level boundaries', () => {
  it('archer tower max level matches config', () => {
    expect(isMaxLevel('archer', TowerUpgradeBalance.MAX_LEVEL)).toBe(true);
  });
  ...
  it('level below max is not max level', () => {
    expect(isMaxLevel('archer', 1)).toBe(false);
    expect(isMaxLevel('archer', 2)).toBe(false);
  });

  it('level above max is still treated as max level', () => {
    expect(isMaxLevel('archer', TowerUpgradeBalance.MAX_LEVEL + 1)).toBe(true);
  });
});

describe('tower upgrade cost resolution', () => {
  it('level 1 has no upgrade cost (initial build)', () => {
    expect(getUpgradeCost('archer', 1)).toBe(TowerUpgradeBalance.LEVEL_2_COST_GOLD);
  });

  it('level 2 upgrade cost points to level 3 cost', () => {
    expect(getUpgradeCost('archer', 2)).toBe(TowerUpgradeBalance.LEVEL_3_COST_GOLD);
  });

  it('max level returns null upgrade cost', () => {
    expect(getUpgradeCost('archer', TowerUpgradeBalance.MAX_LEVEL)).toBeNull();
  });
...
describe('tower upgrade affordability', () => {
  it('allows upgrade when gold is sufficient', () => {
    const result = canAffordUpgrade('archer', 1, TowerUpgradeBalance.LEVEL_2_COST_GOLD);
    expect(result.allowed).toBe(true);
  });

  it('rejects upgrade when gold is insufficient', () => {
    const result = canAffordUpgrade('archer', 1, TowerUpgradeBalance.LEVEL_2_COST_GOLD - 1);
    expect(result.allowed).toBe(false);
    expect(result.reason).toBe('insufficient_gold');
  });

  it('rejects upgrade at max level regardless of gold', () => {
    const result = canAffordUpgrade('archer', TowerUpgradeBalance.MAX_LEVEL, 9999);
    expect(result.allowed).toBe(false);
    expect(result.reason).toBe('max_level');
  });

  it('exact gold amount allows upgrade', () => {
    const result = canAffordUpgrade('archer', 2, TowerUpgradeBalance.LEVEL_3_COST_GOLD);
    expect(result.allowed).toBe(true);
  });

  it('one gold short rejects upgrade', () => {
    const result = canAffordUpgrade('archer', 2, TowerUpgradeBalance.LEVEL_3_COST_GOLD - 1);
    expect(result.allowed).toBe(false);
    expect(result.reason).toBe('insufficient_gold');
  });
});
...
describe('tower stats per level', () => {
  it('returns level 1 stats matching base combat config', () => {
    const stats = getTowerStatsForLevel('archer', 1);
  ...
  it('returns improved stats at level 2', () => {
    const stats = getTowerStatsForLevel('archer', 2);
    expect(stats).not.toBeNull();
  ...
  it('returns max improved stats at level 3', () => {
    const stats = getTowerStatsForLevel('archer', 3);
    expect(stats).not.toBeNull();
  ...
  it('returns null for non-existent level', () => {
    expect(getTowerStatsForLevel('archer', 0)).toBeNull();
    expect(getTowerStatsForLevel('archer', TowerUpgradeBalance.MAX_LEVEL + 1)).toBeNull();
  });
...
describe('tower total invested gold', () => {
  it('level 1 has zero upgrade investment', () => {
    expect(getTotalInvestedGold('archer', 1)).toBe(0);
  });

  it('level 2 includes level 2 upgrade cost', () => {
    expect(getTotalInvestedGold('archer', 2)).toBe(TowerUpgradeBalance.LEVEL_2_COST_GOLD);
  });

  it('level 3 includes both upgrade costs', () => {
    expect(getTotalInvestedGold('archer', 3)).toBe(
...
describe('tower sell value', () => {
  it('level 1 sell value is 50% of build cost', () => {
    const buildCost = 50;
    expect(getSellValue('archer', 1, buildCost)).toBe(Math.floor(buildCost * 0.5));
  });

  it('level 2 sell value includes upgrade cost', () => {
    const buildCost = 50;
    const totalInvested = buildCost + TowerUpgradeBalance.LEVEL_2_COST_GOLD;
    expect(getSellValue('archer', 2, buildCost)).toBe(Math.floor(totalInvested * 0.5));
  });
  ...
  it('level 3 sell value includes all costs', () => {
    ...
    expect(getSellValue('archer', 3, buildCost)).toBe(Math.floor(totalInvested * 0.5));
  });
});

describe('tower upgrade config completeness', () => {
  it('archer has exactly MAX_LEVEL entries', () => {
    expect(TOWER_UPGRADE_CONFIG.archer.levels).toHaveLength(TowerUpgradeBalance.MAX_LEVEL);
  });
  ...
  it('all level entries have sequential levels starting at 1', () => {
    for (const towerType of ['archer', 'splash'] as const) {
